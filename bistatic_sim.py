...
def makeRadarStruct(txPos,rxPos,modes=[False,False,False]):
    radar = {}
    ...
    radar['tau'] = 1.0e-6 #Pulse width (s)
    ...
    #radar['txAz'] = np.array([0, 1]) #Transmit azimuths
    radar['txEl'] = np.zeros(radar['txAz'].shape) #Electrical tx steering elv.
    #radar['txEl'] = np.linspace(0,90,91) #Electrical tx steering elv.
    #radar['txG'] = 45.5 #Transmit antenna gain (dBi)
    radar['txG'] = 43.43 #Transmit antenna gain (dBi)
    radar['rxG'] = 15.5 #Receive antenna gain (dBi)
    radar['receiverGain'] = 60 #Receiver gain (dB)
...
def makeWxStruct():
    ...
    wx['ySize'] = 40e3
    wx['zSize'] = 1e3
    #wx['xSize'] = 5e3 #x,y,z dimensions of simulation volume (m)
    #wx['ySize'] = 40e3
    #wx['zSize'] = 17e3
    wx['getZh'] = 'Zh=35*np.ones((1,nPts))' #Zh/Zv values for uniform test case
    wx['getZv'] = 'Zv=Zh'  
    wx['wrfFile'] = 'cm1out_000007.nc'
    wx['wrfDate'] = '2013-06-01 01:20:10' #NWP data query date: yyyy-mm-dd HH:MM:SS
    wx['wrfOffset'] = np.array([-10.0,20.0,0.25]) # offset of simulation volume np.array([-16,-6,0.5])
...
def getWrf(fName,xq,yq,zq,ptypes):
    ...
    xNames = ['x','xh']
    xName = [name for name in xNames if name in varNames][0]
    XX = np.squeeze(fh.variables[xName][:])
    yNames = ['y','yh']
    yName = [name for name in yNames if name in varNames][0]
    YY = np.squeeze(fh.variables[yName][:])
    zNames = ['z','zh']
...
def getUlaWts(theta,phi,rxG,orient):
#Unit tested
...
# ula) through interpolation from table in rxPat
    ...
    if orient == 'horizontal':
        ...
        inter = interpolate.RegularGridInterpolator((UU[0,:],VV[:,0]),rxPatr)
        wtsr = inter(np.array([uu,vv]).T)
        inter = interpolate.RegularGridInterpolator((UU[0,:],VV[:,0]),rxPati)
        wtsi = inter(np.array([uu,vv]).T)
    wts = wtsr+1j*wtsi
    return wts
...
def simulate(radarStruct,wxStruct):
    ...
    data = loadmat('error_pattern_Norm_final_Crapy1.mat')
    patMatrix = np.zeros((128,128,4)).astype('complex128')
    for i in range(4):
    	txPat = data['pat2']
    	patMatrix[:,:,i] = txPat*10**(txG/20)/np.max(txPat)
    	#patMatrix[:,:,i] = txPat  
        ...
        for ll in range(nEl):
            ...
            for qq in range(M): #For samples in dwell
                ...
                for ii in range(nRx): #For each receiver          
                    ...
                    print(np.max(rxTheta[ii,:]), np.max(rxPhi[ii,:]), rxr[ii,:])
                    ...
                    if ii < nRx-1:
                        Ae = getUlaWts(rxTheta[ii,:],rxPhi[ii,:],rxG,rxOrient)
            ...
            for qq in range(M): #For samples in dwell
                ...
                for ii in range(nRx): #For each receiver          
                    ...
                    if att and tmMode:
                        ...
                        staticVwts[ii,:] *= 10**(-piavr[ii,:]/10)*np.exp(1j*np.deg2rad(khr[ii,:]))
                    if ii < nRx-1:
                        Ae = getUlaWts(rxTheta[ii,:],rxPhi[ii,:],rxG,rxOrient)*_lambda**2/(4*np.pi)
                        staticHwts[ii,:] *= np.sqrt(Ae)
                        staticVwts[ii,:] *= np.sqrt(Ae)  
                        #staticVwts[ii,:] = np.sqrt(vwt*ptVolume*cn2eta(_lambda,cn,thetaS)*Ae)/(4*np.pi*rxr[ii,:,:])
                    # Get total distance traveled from tx to each point back to rx
...
x = np.array([0.0,0.0])*1e3
y = np.array([3.0,0.0])*1e3
z = np.zeros(len(x))
# Tx position
txPos = np.array([0.0,-20.0,0.0])*1e3
...
if saveMode:
    ...
    if len(radarStruct['txMechEl']) > 1:
        fName = f'{band}_{radarStruct["rxPos"].shape[0]}rx_{len(radarStruct["txAz"])}az_{radarStruct["txMechEl"][0]}-{radarStruct["txMechEl"][-1]}el_{radarStruct["M"]}M_{mode}_{2*xSize}km+{mode}+{att}_crap1.xz'
    else:
        fName = f'{band}_{radarStruct["rxPos"].shape[0]}rx_{len(radarStruct["txAz"])}az_{radarStruct["txMechEl"]}el_{radarStruct["M"]}M_{2*xSize}km+{mode}+{att}_crap1.xz'
