...
#file = 'S_2rx_721az_[0.5]el_16M_100000.0km+unwhitened+A_ideal.xz'
#print(file)
...
#im = ax.ravel()[0].pcolormesh(xx[1][:,0,:]/1e3,yy[1][:,0,:]/1e3,10*np.log10(hpp[1][:,0,:]) + 45,vmin=-30,vmax=90,cmap=z_cmap)
#plt.colorbar(im,ax=ax.ravel()[0],label=r'Z_{H} (dBZ)')
#ax.ravel()[0].set_xlim(-20,20)
#ax.ravel()[0].set_ylim(0,40)
#im = ax.ravel()[1].pcolormesh(xx[1][:,0,:]/1e3,yy[1][:,0,:]/1e3,10*np.log10(hpp[1][:,0,:]/vpp[1][:,0,:]),vmin=-4,vmax=8,cmap=z_cmap)
#plt.colorbar(im,ax=ax.ravel()[1],label=r'Z_{DR} (dB)')
#ax.ravel()[1].set_xlim(-20,20)
#ax.ravel()[1].set_ylim(0,40)
#im = ax.ravel()[2].pcolormesh(xx[1][:,0,:]/1e3,yy[1][:,0,:]/1e3,phi[1][:,0,:],vmin=0,vmax=60,cmap='CMRmap_r')
#plt.colorbar(im,ax=ax.ravel()[2],label=r'\phi_{DP} (degree)')
#ax.ravel()[2].set_xlim(-20,20)
#ax.ravel()[2].set_ylim(0,40)
#plt.tight_layout()
#plt.show()
